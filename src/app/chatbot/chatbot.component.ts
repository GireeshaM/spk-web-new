...
export class ChatbotComponent {
  isChatOpen = false; // Controls the chatbot's visibility
  userMessage: string = '';
  messages: Message[] = [];
  isInputEnabled = false;
  showOptions: boolean = true; // Show the initial options when the chatbot opens
  ...
  handleOptionClick(option: string) {
    ...
    switch (option) {
      ...
      case 'jobOpportunities':
        ...
        }, 1500); // Delay to simulate two separate messages
        break;
      case 'connectWithUs':
        this.messages.push({
          sender: 'user',
  ...
  simulateBotResponse(responseText: string, isHtml: boolean = false) {
    setTimeout(() => {
